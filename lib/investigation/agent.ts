...
export function investigate(
  ...
  const paymentRef = payment?.orderId ?? null;
  ...
  steps.push({ label: "References compared", status: referenceMatch === true ? "complete" : referenceMatch === false ? "warning" : "info", detail: paymentRef ? `Payment order: ${paymentRef}` : "Payment reference not available" });
  ...
  const evidenceSufficient = hasAmount && hasCandidates && amountMatch === true;
  steps.push({ label: "Evidence assessment", status: evidenceSufficient ? "complete" : "warning", detail: evidenceSufficient ? "SUFFICIENT" : `INSUFFICIENT — ${[!hasAmount && "no payment amount", !hasCandidates && "no settlement candidates", hasCandidates && amountMatch === false && "amount mismatch"].filter(Boolean).join("; ")}` });
  ...
  if (evidenceSufficient && settlementCandidates.length === 1) {
    recommendation = "MATCH_CANDIDATE"; confidence = 0.85; reason = "Single settlement candidate with exact amount match"; humanReviewRequired = false;
  ...
  const controlPlan = generateControlPlan(recommendation, settlementCandidates, amountMatch, hasAmount, hasCandidates, settlementDateAvailable);
  const whyUnresolved = recommendation === "REVIEW" ? generateWhyUnresolved(settlementCandidates, amountMatch, hasAmount, hasCandidates) : null;
  const whatWouldResolve = generateWhatWouldResolve(recommendation, settlementCandidates, amountMatch, hasAmount, hasCandidates, settlementDateAvailable, paymentRef);
...
function generateWhyUnresolved(candidates: { id: string; amount: number }[], amountMatch: boolean | null, hasAmount: boolean, hasCandidates: boolean): string {
...
function generateWhatWouldResolve(recommendation: "MATCH_CANDIDATE" | "REVIEW", candidates: { id: string }[], amountMatch: boolean | null, hasAmount: boolean, hasCandidates: boolean, settlementDateAvailable: boolean, paymentRef: string | null): string[] {
  const items: string[] = [];
